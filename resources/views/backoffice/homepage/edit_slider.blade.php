            ...
            <form action="{{ route('homepage.updateSlider', $slider) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card mb-6" style="border-radius: 16px; box-shadow: 0 8px 26px rgba(0,0,0,0.03); border: 1px solid rgba(0,0,0,0.05);">
                    <div class="card-header d-flex justify-content-between align-items-center border-bottom py-4">
                        <div>
                            <h4 class="mb-0 text-dark fw-bold">Edit Banner Slide</h4>
                            <span class="text-muted small">Customize slide background, text typography, alignment, and button styling</span>
                        </div>
                    ...
                    <div class="card-body pt-5">
                        ...
                        <div class="row g-4 mb-5">
                            ...
                            <div class="col-lg-5">
                                <label class="form-label fw-bold text-secondary mb-2">Background Graphic (Click to Change)</label>
                                <div class="p-3 rounded text-center d-flex flex-column align-items-center justify-content-center position-relative" style="background-color: #f8f9fa; border: 2px dashed #d9dee3; height: 210px; border-radius: 12px; cursor: pointer;">
                                    <div class="image-preview-container-modal w-100 h-100 d-flex align-items-center justify-content-center" id="image-preview">
                                        @if($slider->image_path)
                                            <img id="bgPreviewImg" src="{{ asset($slider->image_path) }}" alt="Slider Banner" style="max-width: 100%; max-height: 100%; object-fit: cover; border-radius: 8px;">
                                        @else
                                            <span class="text-muted small italic">No image selected.</span>
                                        @endif
                                    </div>
                                    <input class="form-control position-absolute opacity-0" type="file" name="image" id="image" accept="image/*" onchange="previewFile()" style="height: 100%; width: 100%; cursor: pointer; top:0; left:0; z-index: 10;">
                                </div>
                                <span class="text-muted small d-block mt-2 text-center">Click box to upload new banner photo (JPG, PNG, WEBP).</span>
                            </div>
                            ...
                            <div class="col-lg-7">
                                ...
                                <div id="liveBannerCanvas" class="rounded p-4 position-relative overflow-hidden d-flex align-items-center" style="height: 210px; border-radius: 12px; background-size: cover; background-position: center; background-image: url('{{ asset($slider->image_path) }}');">
                                    <div id="liveBannerOverlay" class="position-absolute w-100 h-100" style="top:0; left:0; background: rgba(0,0,0, {{ ($slider->overlay_opacity ?? 0)/100 }}); z-index: 1;"></div>
                                    <div id="liveBannerContent" class="w-100 position-relative" style="z-index: 2;">
                                        <div id="liveSubtitle" class="d-inline-block small px-2 py-1 mb-1 fw-bold" style="font-size: 0.72rem; border-radius: 12px; color: {{ $slider->subtitle_color ?: '#ffffff' }}; background-color: {{ $slider->subtitle_bg ?: 'rgba(255,124,139,0.3)' }}; {{ $slider->subtitle ? '' : 'display: none !important;' }}">{{ $slider->subtitle ?: 'Subtitle Badge' }}</div>
                                        <h5 id="liveTitle" class="fw-bold mb-1" style="color: {{ $slider->title_color ?: '#ffffff' }}; font-size: 1.15rem; line-height: 1.2;">{!! $slider->title ? strip_tags($slider->title) : '' !!}</h5>
                                        <p id="liveDesc" class="small mb-2" style="color: {{ $slider->description_color ?: '#e2e8f0' }}; font-size: 0.8rem; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; {{ $slider->description ? '' : 'display: none !important;' }}">{{ $slider->description }}</p>
                                        <div id="liveButtons" class="d-flex gap-2 align-items-center">
                                            <span id="liveBtn1" class="btn btn-xs fw-bold px-3 py-1" style="border-radius: 20px; font-size: 0.75rem; {{ $slider->btn1_text ? '' : 'display: none !important;' }}">{{ $slider->btn1_text ?: 'Button 1' }}</span>
                                            <span id="liveBtn2" class="btn btn-xs fw-bold px-3 py-1" style="border-radius: 20px; font-size: 0.75rem; {{ $slider->btn2_text ? '' : 'display: none !important;' }}">{{ $slider->btn2_text ?: 'Button 2' }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select class="form-select" id="content_position" name="content_position" onchange="updateLivePreview()">
                                        <option value="left" {{ old('content_position', $slider->content_position ?? 'left') === 'left' ? 'selected' : '' }}>⬅️ Left Aligned</option>
                                        <option value="center" {{ old('content_position', $slider->content_position) === 'center' ? 'selected' : '' }}>⏺️ Center / Middle</option>
                                        <option value="right" {{ old('content_position', $slider->content_position) === 'right' ? 'selected' : '' }}>➡️ Right Aligned</option>
                                    </select>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select class="form-select" id="text_align" name="text_align" onchange="updateLivePreview()">
                                        <option value="left" {{ old('text_align', $slider->text_align ?? 'left') === 'left' ? 'selected' : '' }}>Left Text</option>
                                        <option value="center" {{ old('text_align', $slider->text_align) === 'center' ? 'selected' : '' }}>Center Text</option>
                                        <option value="right" {{ old('text_align', $slider->text_align) === 'right' ? 'selected' : '' }}>Right Text</option>
                                    </select>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select class="form-select" id="overlay_opacity" name="overlay_opacity" onchange="updateLivePreview()">
                                        <option value="0" {{ old('overlay_opacity', $slider->overlay_opacity ?? 0) == 0 ? 'selected' : '' }}>0% (No Dark Tint)</option>
                                        <option value="20" {{ old('overlay_opacity', $slider->overlay_opacity) == 20 ? 'selected' : '' }}>20% (Subtle Light Tint)</option>
                                        <option value="40" {{ old('overlay_opacity', $slider->overlay_opacity) == 40 ? 'selected' : '' }}>40% (Medium Tint - Recommended)</option>
                                        <option value="60" {{ old('overlay_opacity', $slider->overlay_opacity) == 60 ? 'selected' : '' }}>60% (Strong Dark Tint)</option>
                                    </select>
                        ...
                        <div class="mb-5">
                            ...
                            <div class="row g-3 mb-4">
                                <div class="col-md-8">
                                    ...
                                    <div class="input-group input-group-merge">
                                        ...
                                        <input type="text" class="form-control" id="title" name="title" value="{{ old('title', $slider->title) }}" placeholder="e.g. Elegant Corporate Hampers" oninput="updateLivePreview()">
                                    </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="input-group">
                                        <input type="color" class="form-control form-control-color" id="title_color_picker" value="{{ old('title_color', $slider->title_color ?: '#ffffff') }}" style="width: 48px; padding: 4px;" onchange="syncColorInput('title_color', this.value)">
                                        <input type="text" class="form-control" id="title_color" name="title_color" value="{{ old('title_color', $slider->title_color ?: '#ffffff') }}" placeholder="#ffffff" oninput="syncColorPicker('title_color_picker', this.value)">
                                    </div>
                            ...
                            <div class="row g-3 mb-4">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control" id="subtitle" name="subtitle" value="{{ old('subtitle', $slider->subtitle) }}" placeholder="e.g. Exclusive Hamper" oninput="updateLivePreview()">
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label small fw-bold" for="subtitle_color">Badge Text Color</label>
                                    <div class="input-group">
                                        <input type="color" class="form-control form-control-color" id="subtitle_color_picker" value="{{ old('subtitle_color', $slider->subtitle_color ?: '#ffffff') }}" style="width: 44px; padding: 4px;" onchange="syncColorInput('subtitle_color', this.value)">
                                        <input type="text" class="form-control" id="subtitle_color" name="subtitle_color" value="{{ old('subtitle_color', $slider->subtitle_color ?: '#ffffff') }}" placeholder="#ffffff" oninput="syncColorPicker('subtitle_color_picker', this.value)">
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label small fw-bold" for="subtitle_bg">Badge Background</label>
                                    <div class="input-group">
                                        <input type="color" class="form-control form-control-color" id="subtitle_bg_picker" value="{{ old('subtitle_bg', $slider->subtitle_bg ?: '#ff7c8b') }}" style="width: 44px; padding: 4px;" onchange="syncColorInput('subtitle_bg', this.value)">
                                        <input type="text" class="form-control" id="subtitle_bg" name="subtitle_bg" value="{{ old('subtitle_bg', $slider->subtitle_bg ?: '#ff7c8b') }}" placeholder="#ff7c8b" oninput="syncColorPicker('subtitle_bg_picker', this.value)">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row g-3 mb-3">
                                <div class="col-md-8">
                                    ...
                                    <textarea class="form-control" id="description" name="description" rows="2" placeholder="Brief description to capture customer interest..." oninput="updateLivePreview()">{{ old('description', $slider->description) }}</textarea>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label small fw-bold" for="description_color">Description Text Color</label>
                                    <div class="input-group">
                                        <input type="color" class="form-control form-control-color" id="description_color_picker" value="{{ old('description_color', $slider->description_color ?: '#e2e8f0') }}" style="width: 48px; padding: 4px;" onchange="syncColorInput('description_color', this.value)">
                                        <input type="text" class="form-control" id="description_color" name="description_color" value="{{ old('description_color', $slider->description_color ?: '#e2e8f0') }}" placeholder="#e2e8f0" oninput="syncColorPicker('description_color_picker', this.value)">
                                    </div>
                                    <div class="mt-1 d-flex gap-1">
                                ...
                                <div class="col-md-6 border-end">
                                    <div class="p-3 bg-light rounded-3 border h-100">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" id="btn1_text" name="btn1_text" value="{{ old('btn1_text', $slider->btn1_text) }}" placeholder="e.g. Shop Now" oninput="updateLivePreview()">
                                        </div>

                                        <div class="mb-3">
                                            <label class="form-label small fw-semibold">Button 1 Link / URL</label>
                                            <input type="text" class="form-control" name="btn1_link" value="{{ old('btn1_link', $slider->btn1_link) }}" placeholder="e.g. /shop or https://...">
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select" id="btn1_style" name="btn1_style" onchange="onBtn1StyleChange(this.value)">
                                                <option value="theme-primary" {{ old('btn1_style', $slider->btn1_style ?? 'theme-primary') === 'theme-primary' ? 'selected' : '' }}>🌸 Theme Rose Pink Solid (Default)</option>
                                                <option value="dark-solid" {{ old('btn1_style', $slider->btn1_style) === 'dark-solid' ? 'selected' : '' }}>🖤 Charcoal Black Solid</option>
                                                <option value="white-solid" {{ old('btn1_style', $slider->btn1_style) === 'white-solid' ? 'selected' : '' }}>🤍 Crisp White Solid (Dark Text)</option>
                                                <option value="gold-solid" {{ old('btn1_style', $slider->btn1_style) === 'gold-solid' ? 'selected' : '' }}>👑 Royal Gold Solid</option>
                                                <option value="outline-pink" {{ old('btn1_style', $slider->btn1_style) === 'outline-pink' ? 'selected' : '' }}>⭕ Pink Border Outline</option>
                                                <option value="outline-white" {{ old('btn1_style', $slider->btn1_style) === 'outline-white' ? 'selected' : '' }}>⚪ White Border Outline</option>
                                                <option value="outline-dark" {{ old('btn1_style', $slider->btn1_style) === 'outline-dark' ? 'selected' : '' }}>⚫ Dark Border Outline</option>
                                                <option value="custom" {{ old('btn1_style', $slider->btn1_style) === 'custom' ? 'selected' : '' }}>🎨 Custom Colors (Specify Below)</option>
                                            </select>
                                        </div>

                                        <!-- Custom Button 1 Colors -->
                                        <div id="btn1_custom_colors_wrap" class="row g-2 {{ old('btn1_style', $slider->btn1_style) === 'custom' ? '' : 'd-none' }}">
                                            <div class="col-6">
                                                <label class="form-label extra-small fw-semibold">Custom Background</label>
                                                <input type="text" class="form-control form-control-sm" id="btn1_bg_color" name="btn1_bg_color" value="{{ old('btn1_bg_color', $slider->btn1_bg_color) }}" placeholder="#ff7c8b" oninput="updateLivePreview()">
                                            </div>
                                            <div class="col-6">
                                                <label class="form-label extra-small fw-semibold">Custom Text Color</label>
                                                <input type="text" class="form-control form-control-sm" id="btn1_text_color" name="btn1_text_color" value="{{ old('btn1_text_color', $slider->btn1_text_color) }}" placeholder="#ffffff" oninput="updateLivePreview()">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="p-3 bg-light rounded-3 border h-100">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" id="btn2_text" name="btn2_text" value="{{ old('btn2_text', $slider->btn2_text) }}" placeholder="e.g. View Details" oninput="updateLivePreview()">
                                        </div>

                                        <div class="mb-3">
                                            <label class="form-label small fw-semibold">Button 2 Link / URL</label>
                                            <input type="text" class="form-control" name="btn2_link" value="{{ old('btn2_link', $slider->btn2_link) }}" placeholder="e.g. /about or #...">
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select" id="btn2_style" name="btn2_style" onchange="onBtn2StyleChange(this.value)">
                                                <option value="theme-outline" {{ old('btn2_style', $slider->btn2_style ?? 'theme-outline') === 'theme-outline' ? 'selected' : '' }}>🌸 Theme Rose Pink Outline (Default)</option>
                                                <option value="white-outline" {{ old('btn2_style', $slider->btn2_style) === 'white-outline' ? 'selected' : '' }}>⚪ White Border Outline</option>
                                                <option value="dark-outline" {{ old('btn2_style', $slider->btn2_style) === 'dark-outline' ? 'selected' : '' }}>⚫ Dark Border Outline</option>
                                                <option value="theme-primary" {{ old('btn2_style', $slider->btn2_style) === 'theme-primary' ? 'selected' : '' }}>🌸 Theme Rose Pink Solid</option>
                                                <option value="dark-solid" {{ old('btn2_style', $slider->btn2_style) === 'dark-solid' ? 'selected' : '' }}>🖤 Charcoal Black Solid</option>
                                                <option value="white-solid" {{ old('btn2_style', $slider->btn2_style) === 'white-solid' ? 'selected' : '' }}>🤍 Crisp White Solid (Dark Text)</option>
                                                <option value="gold-solid" {{ old('btn2_style', $slider->btn2_style) === 'gold-solid' ? 'selected' : '' }}>👑 Royal Gold Solid</option>
                                                <option value="custom" {{ old('btn2_style', $slider->btn2_style) === 'custom' ? 'selected' : '' }}>🎨 Custom Colors (Specify Below)</option>
                                            </select>
                                        </div>

                                        <!-- Custom Button 2 Colors -->
                                        <div id="btn2_custom_colors_wrap" class="row g-2 {{ old('btn2_style', $slider->btn2_style) === 'custom' ? '' : 'd-none' }}">
                                            <div class="col-6">
                                                <label class="form-label extra-small fw-semibold">Custom Background</label>
                                                <input type="text" class="form-control form-control-sm" id="btn2_bg_color" name="btn2_bg_color" value="{{ old('btn2_bg_color', $slider->btn2_bg_color) }}" placeholder="transparent" oninput="updateLivePreview()">
                                            </div>
                                            <div class="col-6">
                                                <label class="form-label extra-small fw-semibold">Custom Text Color</label>
                                                <input type="text" class="form-control form-control-sm" id="btn2_text_color" name="btn2_text_color" value="{{ old('btn2_text_color', $slider->btn2_text_color) }}" placeholder="#ffffff" oninput="updateLivePreview()">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control" id="orders" name="orders" value="{{ old('orders', $slider->orders) }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label small fw-bold" for="status">Status</label>
                                <select class="form-select" id="status" name="status">
                                    <option value="1" {{ old('status', $slider->status) == '1' ? 'selected' : '' }}>Active (Visible on Frontend)</option>
                                    <option value="2" {{ old('status', $slider->status) == '2' ? 'selected' : '' }}>Inactive (Draft / Hidden)</option>
                                </select>
                            </div>
                        </div>
...
function previewFile() {
    ...
    reader.addEventListener("load", function () {
        document.getElementById('bgPreviewImg').src = reader.result;
...
function updateLivePreview() {
    ...
    var subtitleBg = document.getElementById('subtitle_bg').value || 'rgba(255,124,139,0.3)';
